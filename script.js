  ...
  if (e.localName === "button") {
    ...
    if (e.textContent.toLowerCase() === "all") {
      ...
      arrProducts.forEach((x) => {
        if (x.type === e.textContent.toLowerCase().trim()) {
          display(x);
        }
      });
      addToWishlist();
...
function displayWideVue(product) {
  ...
  <div class="product__images relative overflow-x-scroll flex snap-mandatory snap-x">
    <img id="0"
    ...
    <img id="2"
      ...
      alt=""
      />
    <div class="bullets items-end h-fit sticky top-[calc(100%-1rem)] right-0 min-w-full flex justify-center py-2  bg-white/40 gap-5">
  ...
  <div class="product__data pt-8 pb-4">
    <figcaption
      class="w-product__description mb-4 text-[1.8rem] leading-relaxed"
      >
      ${product.description}
    </figcaption>
    ...
    <div class="product-size mb-4">
      <label
        ...
        >
      <select
        class="sizes w-full p-3 text-[1.6rem] border-[1px] border-black border-solid"
        name="sizes"
        id="size-select"
        aria-label=""
        required
    ...
    <fieldset class="product__colors">
      ...
      <div class="colors__container flex gap-2">
        <div data-color="red" class="product__color w-12 h-12 bg-red-600"></div>
        <div data-color="black" class="product__color w-12 h-12 bg-black"></div>
        <div data-color="orange" class="product__color w-12 h-12 bg-yellow-600"></div>
        <div data-color="blue" class="product__color w-12 h-12 bg-blue-700"></div>
      </div>
    </fieldset>
    <button
      class="btn__checkout  tracking-wider active:translate-y-[1.5px] block w-full my-4 py-4 bg-blue-800 text-white text-[1.8rem] uppercase font-bold"
      type="submit"
      >
    checkout
    </button>
  ...
  slider.addEventListener("touch", function () {
    const bullets = document.querySelectorAll(".bullet");
    const images = document.querySelectorAll(".product__images img");
    images.forEach((x) => {
      if (
        x.getBoundingClientRect().x ===
        parseFloat(getComputedStyle(products)["padding-left"])
      ) {
        bullets.forEach((x) => x.classList.remove("bullet--active"));
        bullets[x.id].classList.add("bullet--active");
      }
    });
  });
  ...
  checkout.addEventListener("click", function gotowhatsapp() {
    const url = `https://wa.me/918789529215?text= 
      name: ${productName}%0a
      size: ${size || "taille dyal nas lkbar a 3ami"}%0a
      color: ${color || "gha kima kan loun malna me3roudin"}`;
    window.open(url, "_blank").focus();
    console.log(`
        name: ${productName}
        size: ${size || "taille dyal nas lkbar a 3ami"}
        color: ${color || "gha kima kan loun malna me3roudin"}
        `);
  });
}
...
products?.addEventListener("click", function (e) {
  ...
  console.log(e.classList.contains("icon-heart"));
  console.log();
  if (
    !e.classList.contains("icon__heart--active") &&
    !e.classList.contains("icon-path") &&
    !e.classList.contains("products") &&
    !e.closest(".show-product")
  ) {
